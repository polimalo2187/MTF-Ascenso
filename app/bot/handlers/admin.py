...
async def _render_user_panel(message_or_callback, user_id: int):
    ...
    if hasattr(message_or_callback, "message"):  # CallbackQuery
        await message_or_callback.message.edit_text(db_text, reply_markup=admin_user_actions_kb(user_id))
        await message_or_callback.answer()
    else:  # Message
        await message_or_callback.answer(db_text, reply_markup=admin_user_actions_kb(user_id))
...
async def admin_user_actions(message: Message):
    ...
    await _render_user_panel(message, user_id)
...
async def admin_back_to_user_panel(callback: CallbackQuery):
    ...
    try:
        ...
        return
    await _render_user_panel(callback, user_id)
...
async def admin_tiers_panel(callback: CallbackQuery):
    ...
    if ((u.get("status") or {}).get("state")) == "banned":
        await callback.answer("Usuario baneado.", show_alert=True)
        return
...
async def admin_set_elite_cb(callback: CallbackQuery):
    ...
    ok, msg = await admin_set_elite(user_id, callback.from_user.id, days=days, note="Premio/forzado por admin")
    await callback.answer(msg, show_alert=True)
    # refresca panel
    await admin_tiers_panel(CallbackQuery(id=callback.id, from_user=callback.from_user, chat_instance=callback.chat_instance, message=callback.message, data=f"admin:tiers:{user_id}"))
...
async def admin_set_titan_cb(callback: CallbackQuery):
    ...
    ok, msg = await admin_set_titan(user_id, callback.from_user.id, days=days, note="Premio/forzado por admin")
    await callback.answer(msg, show_alert=True)
    await admin_tiers_panel(CallbackQuery(id=callback.id, from_user=callback.from_user, chat_instance=callback.chat_instance, message=callback.message, data=f"admin:tiers:{user_id}"))
