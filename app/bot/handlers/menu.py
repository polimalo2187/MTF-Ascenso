...
from app.services.tiers_service import get_multiplier, refresh_tiers
...
async def menu_router(callback: CallbackQuery):
    ...
    if action == "points":
        ...
        if state == "blocked":
            ...
            await callback.answer(f"⛔ Estás bloqueado hasta: {_fmt_dt(blocked_until)}", show_alert=True)
            return
        if state == "banned":
        ...
        prem_redeems = int(((user.get("titan") or {}).get("premium_redeems_count")) or 0)
        ...
        text = (
            ...
            f"⚡ Multiplicador tareas: <b>x{mult}</b>\n"
        )
        ...
        if titan_active:
            ...
            text += f"🔁 Premium canjeado: <b>{prem_redeems}</b>\n"
